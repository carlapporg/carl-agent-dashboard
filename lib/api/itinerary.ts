...
export const itineraryApi = {
  async get(parentTaskId: string): Promise<Itinerary | null> {
    ...
    return apiRequest(API_ENDPOINTS.itinerary.get(parentTaskId), {
      schema: itinerarySchema.nullable(),
    });
  },
  ...
  async generate(parentTaskId: string): Promise<Itinerary> {
    ...
    return apiRequest(API_ENDPOINTS.itinerary.generate(parentTaskId), {
      method: "POST",
