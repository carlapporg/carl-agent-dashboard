...
export function AskQuestionPanel({
  ...
  return (
    <div className="flex h-full min-h-88 flex-col overflow-hidden rounded-(--radius-card) border border-border bg-surface shadow-(--shadow-card)">
      ...
      <div className="flex-1 space-y-3 overflow-y-auto px-5 py-4 md:px-6">
        {thread.length === 0 ? (
          <div className="rounded-xl border border-dashed border-border bg-[#f8fafc] px-4 py-8 text-center">
            <p className="text-base font-medium text-foreground-soft">
              No questions yet
            </p>
            <p className="mt-1 text-base text-muted">
          ...
          thread.map((event) => {
            ...
            return (
              <div
                key={event.id}
                className={cn(
                  "max-w-[85%] rounded-2xl px-4 py-3.5 text-base leading-relaxed",
                  fromAgent
      ...
      {suggestedQuestions.length > 0 ? (
        <div className="flex flex-wrap gap-2 border-t border-border bg-[#f8fafc] px-5 py-3.5 md:px-6">
          <span className="w-full text-sm font-semibold uppercase tracking-wide text-muted">
            Quick ask
          </span>
          {suggestedQuestions.map((q) => (
            <button
              ...
              className="rounded-full border border-border bg-surface px-3.5 py-2 text-left text-base text-foreground-soft transition-colors hover:border-accent/40 hover:text-accent"
            >
              {q}
            </button>
          ))}
      ...
      >
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
          <Textarea
            ...
            disabled={pending}
          />
          <Button type="submit" className="shrink-0 sm:h-12" disabled={pending}>
